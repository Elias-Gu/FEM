...
title_var = {'$\dot{u}(\mathbf{x},t) - \Delta u(\mathbf{x}) = 4\pi\sin(\pi x)\sin(\pi y), \mathbf{x} \in \Omega$',...
    ...
    '$u(\mathbf{x},0) = 0$'};
output_file = 'steps/nodes_coo_0.txt';
...
zlim([-10 10]);
gif('myfile.gif', 'frame',gcf); %https://www.mathworks.com/matlabcentral/fileexchange/63239-gif

for step = 1:step_number
    
    output_file = ['steps/nodes_coo_', num2str(step), '.txt'];
    data = dlmread(output_file);
